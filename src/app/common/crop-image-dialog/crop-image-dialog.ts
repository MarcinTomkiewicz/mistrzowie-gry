...
import { CROP_IMAGE_DIALOG_ZOOM } from './crop-image-dialog.config';

function createInitialTransform(): ImageTransform {
  return {
    scale: 1,
    translateUnit: 'px',
  };
...
export class CropImageDialog {
  ...
  private cropDialogCloseReason: 'confirm' | 'cancel' | null = null;
  ...
  protected readonly zoomConfig = CROP_IMAGE_DIALOG_ZOOM;
  ...
  private applyZoom(value: number | null): void {
    const scale = Math.min(
      CROP_IMAGE_DIALOG_ZOOM.max,
      Math.max(CROP_IMAGE_DIALOG_ZOOM.min, value ?? 1),
    );
