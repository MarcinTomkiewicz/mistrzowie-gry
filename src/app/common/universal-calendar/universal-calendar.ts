...
import { UNIVERSAL_CALENDAR_CONFIG } from './universal-calendar.config';
...
export class UniversalCalendar {
  ...
  readonly monthLabel = computed(() =>
    formatMonthLabel(this.currentMonth(), UNIVERSAL_CALENDAR_CONFIG.locale),
  );

  readonly weekdayLabels = computed(() =>
    getWeekdayLabels(
      UNIVERSAL_CALENDAR_CONFIG.locale,
      UNIVERSAL_CALENDAR_CONFIG.weekStartsOn,
    ),
  );
  ...
  private createCalendarDay(date: Date): IUniversalCalendarDayVm {
    ...
    const hours = this.createHours(
      sourceDay?.hours ?? UNIVERSAL_CALENDAR_CONFIG.emptyHours,
    );
    const isUnavailable =
  ...
  private createHours(
    ...
    const totalSlots = Math.max(
      UNIVERSAL_CALENDAR_CONFIG.slotCount,
      sourceHours?.length ?? 0,
    );

    return Array.from({ length: totalSlots }, (_, index) => {
      const isActive = !!sourceHours?.[index];
      const hour = UNIVERSAL_CALENDAR_CONFIG.slotStartHour + index;
