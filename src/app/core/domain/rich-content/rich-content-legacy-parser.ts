import { parseInternalLinkText } from '../internal-link/internal-link-markup';
import type {
  RichContent,
  RichContentBlock,
...
export function parseLegacyRichContent(value: string): RichContent | null {
  const normalized = normalizePlainText(value);
  return normalized ? parsePlainTextToRichContent(normalized) : null;
}
...
function parsePlainTextToRichContent(text: string): RichContent {
  ...
  for (const chunk of chunks) {
    if (isArticleHeading(chunk)) {
      ...
      currentSection = { title: chunk, blocks: [] };
      continue;
    }
...
function parseUnorderedList(
  chunk: string,
): RichContentUnorderedListBlock | null {
  const lines = splitLines(chunk);
