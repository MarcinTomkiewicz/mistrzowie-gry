...
import type { PriceEditorForm } from '../types/price-editor-form';
...
export function createPriceEditorForm(
  ...
  const values = getPriceEditorValues(price);
...
export function mapPriceEditorForm(
  form: PriceEditorForm,
): Price {
...
function getPriceEditorValues(price: Price | null) {
  const values = {
...
function requireNumber(value: number | null): number {
  if (value === null) {
    throw new TypeError('A valid price requires a numeric value.');
  }
